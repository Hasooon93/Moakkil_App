...
async function updateCaseDetails(event) {
    ...
    const data = {
        ...
        confidentiality_level: document.getElementById('edit_confidentiality_level').value,
        current_stage: document.getElementById('edit_current_stage').value,
        ...
        judgment_date: document.getElementById('edit_judgment_date').value || null,
        police_station_ref: document.getElementById('edit_police_station_ref').value,
        prosecution_ref: document.getElementById('edit_prosecution_ref').value,
        ...
        final_requests: parseLinesToArray(document.getElementById('edit_final_requests').value),
        case_outcome: document.getElementById('edit_case_outcome').value,
        ...
        closure_reason: document.getElementById('edit_closure_reason').value,
        physical_archive_location: document.getElementById('edit_physical_archive_location').value,
...
function openUpdateModal() {
    document.getElementById('upd_extracted_json').value = '';
    openModal('updateModal');
}

async function previewAIExtraction() {
    const details = document.getElementById('upd_details').value;
    if(!details) return showAlert('الرجاء كتابة تفاصيل الواقعة أو اللائحة أولاً', 'warning');
    
    const btn = document.getElementById('btn_ai_extract');
    if(btn) { btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> جاري التحليل العميق...'; }
    ...
    try {
        ...
        if(aiRes && aiRes.extracted_json) {
            ...
            if (ex.lawsuit_facts || ex.legal_basis) {
                document.getElementById('upd_extracted_json').value = JSON.stringify(ex);
            }
...
function renderTimeline(updates) {
    ...
    if (!updates || updates.length === 0) { container.innerHTML = '<div class="text-center p-4 text-muted small bg-white rounded border shadow-sm">لا يوجد وقائع مسجلة.</div>'; return; }
    
    container.innerHTML = updates.map(u => {
        const attachHtml = u.attachment_url ? `<a href="${escapeHTML(u.attachment_url)}" target="_blank" class="badge bg-soft-primary text-primary mt-2 border border-primary text-decoration-none shadow-sm p-1 px-2"><i class="fas fa-paperclip"></i> عرض المرفق</a>` : '';
        return `
        <div class="timeline-item mb-3">
            <div class="card-custom p-3 shadow-sm bg-white border-end border-4 border-navy position-relative">
                ...
                ${u.hearing_date ? `<small class="d-block mt-2 text-muted"><i class="fas fa-calendar-check text-success"></i> تاريخ الجلسة: ${escapeHTML(u.hearing_date)}</small>` : ''}
                ${attachHtml}
            </div>
        </div>
    `}).join('');
...
async function loadAuditTrail() {
    ...
    try {
        ...
        if(!history || history.length === 0) { container.innerHTML = '<div class="alert alert-light border text-center small text-muted mb-0">لا يوجد حركات مسجلة.</div>'; return; }
        ...
        container.innerHTML = history.map(h => {
            ...
            return `<div class="border-start border-4 border-${action.color} ps-3 mb-3 pb-2 border-bottom"><small class="text-muted d-block mb-1"><i class="far fa-clock"></i> ${new Date(h.created_at).toLocaleString('ar-EG')}</small><span class="badge bg-${action.color} mb-1">${action.ar}</span><p class="mb-0 small fw-bold text-dark">تحديث على السجل.</p></div>`;
        }).join('');
    } catch(e) { container.innerHTML = '<div class="text-danger small text-center p-2 border border-danger">خطأ في الاتصال بسجل الرقابة.</div>'; }
}
...
function calculateFinances(installments, expenses) {
    ...
    if(netEl) {
        netEl.innerText = clientRemaining.toLocaleString() + ' د.أ';
...
function renderPayments(installments) {
    ...
    if (!installments || installments.length === 0) { container.innerHTML = '<div class="text-center p-3 text-muted small border bg-white rounded">لا توجد دفعات مسجلة.</div>'; return; }
    container.innerHTML = installments.map(i => {
        ...
        const waBtn = !isPaid ? `<button class="btn btn-sm whatsapp-btn py-0 px-2 ms-1 shadow-sm" onclick="sendWhatsAppReminder('${escapeHTML(i.amount)}', '${escapeHTML(i.due_date || new Date(i.created_at).toLocaleDateString('ar-EG'))}')" title="إرسال تذكير عبر واتساب"><i class="fab fa-whatsapp"></i></button>` : '';
...
function renderExpenses(expenses) {
    ...
    if (!expenses || expenses.length === 0) { container.innerHTML = '<div class="text-center p-3 text-muted small border bg-white rounded">لا توجد مصروفات مسجلة.</div>'; return; }
...
function copyToClipboard(elementId) {
    const el = document.getElementById(elementId); el.select(); el.setSelectionRange(0, 99999); document.execCommand("copy");
...
function printInvoice(amount, date, invoiceId) {
    ...
    document.getElementById('print-inv-tafqeet').innerText = tafqeet(parseInt(amount)) + " دينار أردني";
...
function printQRCode() {
    if(!caseObj.public_token) { showAlert('لا يوجد رابط وصول عام.', 'warning'); return; }
...
function copyShareLink() { navigator.clipboard.writeText(`مرحباً، يمكنك متابعة تفاصيل قضيتك عبر الرابط:\n${currentShareLink}\n\nالرمز السري (PIN): ${document.getElementById('share_pin_input').value}`).then(() => showAlert('تم نسخ الرابط', 'success')); }
function sendViaWhatsApp() { window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(`مرحباً، يمكنك متابعة تفاصيل قضيتك عبر الرابط:\n${currentShareLink}\n\nالرمز السري (PIN): ${document.getElementById('share_pin_input').value}`)}`, '_blank'); }

function sendWhatsAppReminder(amount, dueDate) {
    const client = window.firmClients.find(cl => cl.id === caseObj.client_id);
    if (!client || !client.phone) { showAlert('لا يوجد رقم هاتف مسجل للموكل.', 'warning'); return; }
    let phoneStr = String(client.phone);
    if (phoneStr.startsWith('0')) phoneStr = '962' + phoneStr.substring(1);
    const text = `تحية طيبة السيد/ة ${client.full_name}،\n\nنود تذكيركم باستحقاق دفعة مالية بقيمة *${amount} دينار أردني*، بتاريخ (${dueDate}).\nيرجى التواصل معنا لترتيب عملية السداد.\n\nمع التحية،\nإدارة المكتب.`;
