// js/staff.js - محرك إدارة الموارد البشرية والموظفين (HR - Cloudflare R2 Secured)
// التحديثات: إضافة دالة العرض المقروء فقط (View Profile) وفصلها عن دالة التعديل (Edit Profile). دمج التخزين السحابي الجديد R2 وتأمين روابط الوثائق.
...
window.viewStaffDetails = function(id) {
    ...
    if (staff.staff_documents && Array.isArray(staff.staff_documents) && staff.staff_documents.length > 0) {
        docContainer.innerHTML = staff.staff_documents.map(doc => {
            ...
            let secureUrl = doc.url;
            if (doc.url && typeof API !== 'undefined' && typeof API.getSecureUrl === 'function') {
                secureUrl = API.getSecureUrl(doc.url);
            }
            return `
            <div class="doc-item bg-white shadow-sm border-0">
                ...
                <a href="${escapeHTML(secureUrl)}" target="_blank" class="btn btn-sm btn-outline-navy fw-bold rounded-pill px-3"><i class="fas fa-eye"></i> عرض الأصل</a>
            </div>
        `}).join('');
    } else {
...
function renderStaffDocs(docs) {
    ...
    list.innerHTML = docs.map((doc, index) => {
        ...
        <div class="doc-item">
            ...
            <div>
                <a href="${escapeHTML(secureUrl)}" target="_blank" class="btn btn-sm btn-outline-primary px-2 py-0"><i class="fas fa-eye"></i></a>
