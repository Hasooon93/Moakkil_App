...
function renderTimeline(updates) {
    ...
    container.innerHTML = updates.map(u => `<div class="timeline-item mb-3"><div class="card-custom p-3 shadow-sm bg-white border-end border-4 border-navy"><small class="text-primary fw-bold">${new Date(u.created_at).toLocaleDateString('ar-EG')}</small><h6 class="fw-bold text-navy mt-1">${u.update_title}</h6><p class="mb-0 small">${u.update_details}</p></div></div>`).join('');
}
...
function renderPayments(installments) {
    ...
    container.innerHTML = installments.map(i => `<div class="card-custom p-3 mb-2 d-flex justify-content-between align-items-center border-start border-4 ${i.status === 'مدفوعة' ? 'border-success' : 'border-warning'}"><b class="fs-5">${i.amount} د.أ</b><span class="badge ${i.status === 'مدفوعة' ? 'bg-success' : 'bg-warning text-dark'}">${i.status}</span></div>`).join('');
}
...
function renderFiles(files) {
    ...
    container.innerHTML = files.map(f => `<div class="col-6"><div class="card-custom p-3 text-center border shadow-sm"><i class="fas fa-file-pdf fs-1 text-danger mb-2"></i><h6 class="small fw-bold text-truncate">${f.file_name}</h6><a href="${f.drive_file_id}" target="_blank" class="btn btn-sm btn-outline-primary w-100 fw-bold mt-2">عرض</a></div></div>`).join('');
}
