// js/case-details.js - محرك التفاصيل المطور (محدث لتصحيح مصفوفات الإسناد)
...
function openEditModal() {
    ...
    if(lawyerSelect && window.firmStaff) {
        ...
        // إذا كان هناك إسناد سابق، حدده. وإذا كان مصفوفة، خذ العنصر الأول (لتبسيط نافذة التعديل)
...
async function updateCaseDetails(event) {
    ...
    const data = {
        ...
        // إرسال الإسناد كمصفوفة كما تتوقع قاعدة البيانات
...
async function saveUpdate(event) {
    ...
    const data = {
        ...
        // إرسال الإسناد كمصفوفة كما تتوقع قاعدة البيانات في بعض الجداول (أو كنص إذا كان الجدول يقبله، Worker سيتعامل معه)
